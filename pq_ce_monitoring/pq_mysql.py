...
from accounting import Errors
from logger import ServiceLogger

_logger = ServiceLogger("pq_mysql").logger
    ...
    def write_data(self, tdelta):
        ...
        for hit in s.aggregations.computingelement:
            harvester_computingelements_list_df.append(
                [date_key.strftime("%Y-%m-%dT%H:%M")[:-1] + '0:00', hit.key, hit.doc_count,
                 hit.workerstats.buckets['good']['doc_count'],
                ...
                if queuename in q_pq_keys:
                    for error in harvester_queues_errors[queuename]:
                        harvester_queues_errors_list_df.append([date_key.strftime("%Y-%m-%dT%H:%M")[:-1] + '0:00',
                                                                ...
                                                                queuename,
                                                                error,
                                                                harvester_queues_errors[queuename][error][
                                                                    'error_count'],
                                                                queue['harvester']
                                                                ])
            else:
        ...
        try:
            frame_computingsite = pd.DataFrame(harvester_queues_list_df,
                                               columns=["timestamp", "computingsite", "cloud", "site", "totalworkers",
                                                        "goodworkers", "badworkers", "ratio", "status", "harvesterid"])
            frame_computingsite.to_sql(con=self.connection, name='Computingsite', if_exists='append', index=False,
                                       chunksize=1)
        except Exception as ex:
            _logger.error(ex)
        try:
            frame_computingsite_errors = pd.DataFrame(harvester_queues_errors_list_df,
                                                      columns=["timestamp", "cloud", "site", "computingsite",
                                                               "errordesc", "errorcount", "harvesterid"])
            frame_computingsite_errors.to_sql(con=self.connection, name='Pqerrors', if_exists='append', index=False,
                                              chunksize=1)
        except Exception as ex:
            _logger.error(ex)
